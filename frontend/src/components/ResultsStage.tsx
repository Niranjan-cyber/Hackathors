...
import { Trophy, Download, Mail, RotateCcw, Target, Clock, ChevronDown, ChevronUp, FileText, X, Repeat } from 'lucide-react';
...
const ResultsStage: React.FC<ResultsStageProps> = ({ quizData, setQuizData, setCurrentStage }) => {
  ...
  const handleRetakeSameTopics = () => {
    const uniqueTopics = Array.from(
      new Set((quizData.questions || []).map((q: any) => q.topic).filter(Boolean))
    );
    const topicsToUse =
      uniqueTopics.length === 0 && Array.isArray(quizData.topics)
        ? quizData.topics
        : uniqueTopics;

    // Reset transient quiz state to ensure fresh generation
    setQuizData({
      ...quizData,
      topics: topicsToUse,
      presetTopics: true,
    ...
    // Go back to topic selection with previous choices preselected
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      // Generate PDF blob from the same ResultsPdf component
      const blob = await pdf(<ResultsPdf quizData={quizData} />).toBlob();

      // Create a File from the Blob so filename is preserved on the backend
      const file = new File([blob], 'quiz-report.pdf', { type: 'application/pdf' });

      // Prepare email payload
      const subject = `Your Quiz Report (${quizData?.difficulty ? quizData.difficulty.charAt(0).toUpperCase() + quizData.difficulty.slice(1) : 'Results'})`;
      const percentage = (() => {
        const score = quizData.score || 0;
        const total = (quizData.questions?.length || quizData.count || 0);
        return total > 0 ? Math.round((score / total) * 100) : 0;
      })();
      const body = `Hi,

Please find attached your quiz report.

- Score: ${quizData.score || 0}/${quizData.questions?.length || quizData.count}
- Accuracy: ${percentage}%
- Difficulty: ${quizData?.difficulty || 'N/A'}
- Time Spent: ${quizData?.timeSpent ? `${Math.floor(quizData.timeSpent / 60)}m ${quizData.timeSpent % 60}s` : 'N/A'}

Generated by Neocortex Quiz Platform.`;

      const formData = new FormData();
      formData.append('to', emailInput.trim());
      ...
      toast.success(`Results sent to ${emailInput}!`, { duration: 2000 });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-10 fade-in-up">
        <button onClick={handleRetakeSameTopics} className="premium-button px-6 py-3 flex items-center space-x-2" title="Re-use the same PDF and preselect your previous topics">
          <Repeat className="w-5 h-5" />
          <span>Take test on same PDF</span>
        </button>
